...
import { useEditSkladnikiMutation } from "../../../features/skladnikiApiSlice"
...
export default function Editskladniki({ skladniki }) {
  usePageTitleAdmin(skladniki.name)
  const [name, setName] = useState(skladniki?.name)
  const [producent, setProducent] = useState(skladniki?.producent)
  const [kontrahent, setKontrahent] = useState(skladniki?.kontrahent)
  const [category, setCategory] = useState(skladniki?.category)
  const [cena, setCena] = useState(skladniki?.cena)
  const [tax, setTax] = useState(skladniki?.tax)
  const [invoice, setInvoice] = useState(skladniki?.invoice)

  const navigate = useNavigate()

  const [editSkladniki, { isLoading, isSuccess, isError, error, data }] =
  ...
  const formHandler = async (e) => {
    ...
    await editSkladniki({ editedskladniki })
  }
